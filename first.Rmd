...
date: "`r Sys.Date()`" # will change the date to now at knitting
output:    
  html_document:
    includes:
...
![**_Fig. 9. Pick the type of the project._**](Project11.png)

\
This might be more important when you're more comfortable with R! Not only can you create a project out of already existing directory, but also start a GitHub-related project - this might be useful if you wanted to create a small website with R! You could link your project to a repository in GitHub and further render your RMarkdown file into HTML and publish it! Just as the site you see right now!

# Create your own working directory and a new project
```{r}

```
#File->NewProject->NewDirectory->NewProject
#DirectoryName: ourstudentnumber_R1 ->subdirectory "your folder"-> OK
#Now click on the small icon with a blank piece of paper and a '+'and choose 'R Script'
#If you didn't have the code editor open - now it should open itself! You can add new Folders or Files in the right-bottom part of RStudio, This helps you organize your project!
```

#It is useful to check the Options:
```{r}

#Tools->Global Options
#Where you can not only see/change a current working directory, but also decide if we want to load our last work the next time we open RStudio:
#Workspace: Click/unclick "Restore .RData into workspace at startup"
#and decide if RStudio should Save workspace to .RData on exit!!!
```
